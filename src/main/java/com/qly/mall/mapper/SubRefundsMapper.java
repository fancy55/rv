...
public interface SubRefundsMapper {
    @Insert("insert into sub_refunds(refund_id,sub_refund_id,order_id,sub_order_id,status,refund_amount,refund_type,refund_reason,create_time,update_time,version) values(#{refundId},#{subRefundId},#{orderId},#{subOrderId},#{status},#{refundAmount},#{refundType},#{refundReason},now(),now(),0)")
